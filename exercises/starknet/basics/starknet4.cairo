...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            let stock = self.storage_inventory.read(product) + new_stock;
            self.storage_inventory.write(product, stock);
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            let new_quantity = self.storage_inventory.read(product) - quantity;
            self.storage_inventory.write(product, new_quantity);
        }
